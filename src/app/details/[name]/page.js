...
const DetailsPage = ({ params, searchParams }) => {
  ...
  const [cartIds, setCartIds] = useState(() =>
    cartStorage.map(({ _id }) => _id)
  );
  const [removeCartItem, setRemoveCartItem] = useState("");

  //   console.log("🚀 ~ DetailsPage ~ cartIds:", cartIds);
  ...
  const getRestroDetails = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // console.log("🚀 ~ getRestroDetails ~ foodItems:", foodItems);
  ...
  const removeFromCart = (id) => {
    ...
    // console.log("🚀 ~ removeFromCart ~ cartIds:", cartIds);
    const localIds = cartIds.filter((itemId) => itemId !== id);
    // console.log("🚀 ~ removeFromCart ~ localIds:", localIds);
    setCartIds(localIds);
    setCartItems({});
  };
  ...
  return (
    <div>
      <Header cartItems={cartItems} removeCartItem={removeCartItem} />
      {loader && <Loader />}
      ...
      <div className="foodItemDetails">
        {foodItems.length > 0 ? (
          foodItems?.map(
            ({ _id, name, price, description, imagePath, restoId }) => (
              <div key={_id} className="foodItemList">
                ...
                <div className="foodItemDetailsText">
                  <div className="foodItemDetailsTextBox">
                    <p>Dish : {name}</p>
                    <p>Price : {price}</p>
